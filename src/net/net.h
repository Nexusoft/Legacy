...
namespace Core { class CBlockIndex; }
namespace Wallet { class CAddrDB; }
...
namespace Net
{
	...
	public:
		...
		bool fGetAddr;
		uint1024 hashCheckpointKnown; // Nexus: known sent sync-checkpoint
	...
	public:
		...
		int GetRefCount()
		{
			return std::max(nRefCount, 0) + (GetUnifiedTimestamp() < nReleaseTime ? 1 : 0);
		}

		CNode* AddRef(int64 nTimeout=0)
		{
			if (nTimeout != 0)
				nReleaseTime = std::max(nReleaseTime, GetUnifiedTimestamp() + nTimeout);
			else
				nRefCount++;
			return this;
		}
		...
		void PushGetBlocks(Core::CBlockIndex* pindexBegin, uint1024 hashEnd);
	...
	{
		{
			...
			while (!vRelayExpiration.empty() && vRelayExpiration.front().first < GetUnifiedTimestamp())
			{
			...
			vRelayExpiration.push_back(std::make_pair(GetUnifiedTimestamp() + 15 * 60, inv));
		}
