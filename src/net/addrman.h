...
namespace Net
{
	...
	public:
		...
		bool IsTerrible(int64 nNow = GetUnifiedTimestamp()) const;

		// Calculate the relative chance this entry should be given when selecting nodes to connect to
		double GetChance(int64 nNow = GetUnifiedTimestamp()) const;
	...
	public:
		...
		void Good(const CService &addr, int64 nTime = GetUnifiedTimestamp())
		{
			{
				LOCK(cs);
				Check();
		...
		void Attempt(const CService &addr, int64 nTime = GetUnifiedTimestamp())
		{
			{
				LOCK(cs);
				Check();
		...
		// Mark an entry as currently-connected-to.
		void Connected(const CService &addr, int64 nTime = GetUnifiedTimestamp())
		{
			{
				LOCK(cs);
				Check();
