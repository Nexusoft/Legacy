...
struct TxLessThan
{
    ...
    bool operator()(const TransactionRecord &a, const uint512 &b) const
    {
        return a.hash < b;
    }
    bool operator()(const uint512 &a, const TransactionRecord &b) const
    {
        return a < b.hash;
    }
};
...
{
public:
    TransactionTablePriv(Wallet::CWallet *wallet, TransactionTableModel *parent):
            wallet(wallet),
            parent(parent)
    {
    }
     ...
     * As it is in the same order as the Wallet::CWallet, by definition
    ...
    void refreshWallet()
    {
        ...
        cachedWallet.clear();
        {
            ...
            for(std::map<uint512, Wallet::CWalletTx>::iterator it = wallet->mapWallet.begin(); it != wallet->mapWallet.end(); ++it)
            {
     ...
     */
    void updateWallet(const QList<uint512> &updated)
    {
        ...
        QList<uint512> updated_sorted = updated;
    ...
    TransactionRecord *index(int idx)
    {
        if(idx >= 0 && idx < cachedWallet.size())
        {
            ...
            if(rec->statusUpdateNeeded())
            {
                {
                    LOCK(wallet->cs_wallet);
                    std::map<uint512, Wallet::CWalletTx>::iterator mi = wallet->mapWallet.find(rec->hash);
...
TransactionTableModel::TransactionTableModel(Wallet::CWallet* wallet, WalletModel *parent):
...
void TransactionTableModel::update()
{
    ...
    // Check if there are changes to wallet map
    {
        ...
        {
            BOOST_FOREACH(uint512 hash, wallet->vWalletUpdated)
            {
                updated.append(hash);
            }
...
QString TransactionTableModel::formatTxStatus(const TransactionRecord *wtx) const
{
    ...
    if(wtx->type == TransactionRecord::Generated || wtx->type == TransactionRecord::StakeGenesis || wtx->type == TransactionRecord::StakeTrust)
    {
        ...
        case TransactionStatus::Immature:
            status += tr("Immature (%1 of %2 confirmations)").arg(wtx->status.depth).arg(wtx->status.matures_in + wtx->status.depth) + "\n" + tr("Balance will be available in %n more blocks", "",
...
{
    QString str = NexusUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit);
    if(showUnconfirmed)
    {
