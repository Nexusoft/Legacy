...
namespace Core
{
    ...
    void StakeMinter(void* parg)
    {
        ...
        /* Check for the most recent trust block. */
        uint64 nBestAge = 0;
        ...
        else
        {
            ...
            if(indexdb.GetTrustKeys(vKeys))
            {
                for(auto key : vKeys)
                {
                    ...
                    if(pwalletMain->HaveKey(address))
                    {
                        /* Calculate the trust key's age. */
                        uint64 nAge = trustCheck.BlockAge(hashBestChain);

                        /* Compare to nBestAge. */
                        if(nBestAge == 0 || nAge < nBestAge)
                        {
                            /* Debug output. */
                            printf("Stake Minter : Found my Trust Key %s\n", HexStr(key.begin(), key.end()).c_str());

                            /* Set the best age of the block. */
                            nBestAge = nAge;
