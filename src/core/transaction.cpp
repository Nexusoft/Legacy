                                     ...
                                     const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
    {
        ...
        if (!IsCoinBase())
        {
            ...
            if (IsCoinStake())
            {
                ...
                printf("ConnectInputs() : %f Value Out, %f Expected\n", (double)round_coin_digits(vout[0].nValue, 1) / COIN, (double)(round_coin_digits(nInterest + nValueIn, 1)) / COIN);

                if (round_coin_digits(vout[0].nValue, 1) > round_coin_digits((nInterest + nValueIn), 1))
